...
As explained above, `statcheck` is better equipped to process `HTML` than `PDF` which might lead to a difference in reliability. However, the generalizability of the results increases considerably when we include studies that are only published in `PDF`. To deal with this trade-off, we report `HTML` results separately from `PDF` results and draw conclusions separately.
